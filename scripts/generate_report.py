...
def generate_report(
    ...
    # Sort packages so that failed tests appear first
    def sort_by_failure(pkg: dict[str, Any]) -> tuple[int, str]:
        """Sort key: failed packages first (0), then by package name."""
        # A package is considered failed if status contains "Fail"
        has_failure = "Fail" in pkg.get("original_status", "") or "Fail" in pkg.get(
            "patched_status", ""
        )
        return (0 if has_failure else 1, pkg["name"])

    patched_packages.sort(key=sort_by_failure)
    unpatched_packages.sort(key=sort_by_failure)
    dependency_packages.sort(key=sort_by_failure)
